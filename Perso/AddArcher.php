 ...
 */
	...
	// Vérifier la session avant d'inclure d'autres fichiers
	if (!isset($_SESSION['TourId'])) {
		die("Erreur: Session TourId non définie");
	}
	
	// Inclure les autres fichiers avec gestion d'erreurs
	$files = array(
		'Common/Fun_Various.inc.php',
		'Common/Fun_Sessions.inc.php',
		'Qualification/Fun_Qualification.local.inc.php',
		'Partecipants/Fun_Partecipants.local.inc.php', // CORRIGÉ : dans Partecipants/
	...
	foreach ($files as $file) {
		...
		if (file_exists($fullPath)) {
			...
			die("Erreur: Fichier $fullPath non trouvé");
		}
	}
...
/* salvo */
	if (isset($_REQUEST['Command']))
	{
		...
		// se $ses e $tar non sono nulli cerco il premier paglione libero dopo $ses.$tar
...
/* fine salvo */
	...
	else
	{
		$record=array(
			...
			'targetface' => 0,
			'targetface_name' => '',
			'indcl'=>1,  // Changé de 0 à 1 (Yes pour Ind. Div./Class)
			'teamcl'=>0, // Changé à 0 (No pour Team Div./Class)
			'indfin'=>1, // Changé de 0 à 1 (Yes pour Ind. Events)
			'teamfin'=>0, // Changé à 0 (No pour Team Events)
			'mixteamfin'=>0, // Changé à 0 (No pour Mixed Team Events)
			'wc'=>0,
			'double'=>0,
		);
	}
...
// per i reset js delle combo
	...
	$JS_SCRIPT=array(
		...
		'<script type="text/javascript" src="' . $CFG->ROOT_DIR . 'Partecipants/Fun_AJAX_PopEdit.js"></script>',
	...
	// Utiliser le template standard au lieu du template popup
	include($basePath . '/Common/Templates/head.php');
?>

<form name="Frm" method=post action="<?php print $_SERVER['PHP_SELF'];?>" onsubmit="return false;">
	...
	<div style="margin: 10px 0; padding: 10px; background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 5px;">
		<strong>Note :</strong> Cette page est modifié pour rajouter un Archer dans une compétition simple.
	</div>
...
<div id="finder"> <!-- style="display:none;"-->
		<table class="Tabella">
			...
			<tr>
				<td class="Center" style="width:10%;"><input type="text" size="6" id="findCode" name="findCode" value="<?php echo !empty($_REQUEST['findCode']) ? $_REQUEST['findCode'] : ''; ?>" onkeypress="handleEnterKey(event, 'findAth')" /></td>
				<td class="Center" style="width:25%;"><input type="text" size="25" id="findAth" name="findAth" value="<?php echo !empty($_REQUEST['findAth']) ? $_REQUEST['findAth'] : ''; ?>" onkeypress="handleEnterKey(event, 'findCountry')" /></td>
				<td class="Center" style="width:25%;"><input type="text" size="25" id="findCountry" name="findCountry" value="<?php echo !empty($_REQUEST['findCountry']) ? $_REQUEST['findCountry'] : ''; ?>" onkeypress="handleEnterKey(event, 'findDiv')" /></td>
...
function handleEnterKey(event, nextFieldId) {
    if (event.keyCode === 13) {
        ...
        if (nextFieldId) {
            ...
            // Si c'est le dernier champ, lancer la recherche
            FindArchers();
        }
        return false;
    }
...
// Activer la touche Entrée dans les champs de recherche
document.addEventListener('DOMContentLoaded', function() {
    // Ajouter l'événement keypress aux champs de recherche
    var searchFields = ['findCode', 'findAth', 'findCountry', 'findDiv', 'findCl', 'findSubCl'];
    ...
    // Focus sur le premier champ de recherche si vide
    var findAthField = document.getElementById('findAth');
    if (findAthField && !findAthField.value) {
        findAthField.focus();
    }
});
...
// Utiliser le template standard au lieu du template popup
include($basePath . '/Common/Templates/tail.php');
